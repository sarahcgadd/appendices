#this script was generated using a bash script which created several different R scripts
#with different "fileref" numbers, along with a script to submit the scripts as an array
#to a task scheduler
...
#size of images parameters
width=119
height=195

#set up knot configurations - this refers to settings in earlier code used to choose knot configuration
knotconfigs<-c(rep(19,6))
set.seed(542)

#set up possible values of tau
sds<-c(20,10,5,1,0.5,0.1)
taus<-1/(sds^2)

#data frames to collect results
rankcor<-data.frame(id=1:6, maxmin=NA, maxmin.lower=NA, maxmin.upper=NA, maxtime=NA, maxtime.lower=NA, maxtime.upper=NA)
blandaltstats<-data.frame(id=1:6, delaybias=NA, delaybiasupper=NA, delaybiaslower=NA, 
...
ni <- 4000 #number of iterations first model (no temporal autocorrelation)
nb <- 3000 #number for burn in, first model
nt <- 1 #thinning parameter
ncores<-1 #n cores to use
ni2<-20000 #iterations for second model
nb2<-19000 #burn in for second model
boots<-4000 #number of samples to retain (here, this is (ni2-nb2)*nc, so all posterior samples)
#read data
...
#generate list of connections between all stations
...
#make a variable detailing fixed effects for origin station, based on the number of neighbours in the underground infrastructure
...
journeytimes$length<-journeytimes$length/60 #change to hours from minutes
...
#put in form that openbugs uses
...
#set up time difference information
...
#different knot specifications (used when originally choosing knot spec)
splineref<-list()
for(nknots in 1:20){
...
#set up basis function and derivative
...
##################################
#run first model with no temporal autocorrelation
####################################
...
#set up initial values based on previous results
...
bugs.out <- bugs(data=bugsdata, inits=inits, 
                 parameters.to.save=params, model.file=paste("model",fileref,".txt",sep=""), 
                 n.chains=nc, n.iter=ni2, n.burnin=nb2, n.thin=nt, debug=F, DIC=TRUE)
...
write.csv(return, paste("test_knots",fileref,".csv",sep=""),row.names=F)
...
#Posterior sample parameters
sample<-sample(c(1:((ni2-nb2)*nc)), boots)
iters<-(ni2-nb2)*nc
...
#collect mean values
input<-unique(journeytimes[,c("id","start.station_id","end.station_id","distancekm")])
...
########################################
#set up matrices
max.val.sim<-matrix(nrow=boots, ncol=nrow(input))
max.time.sim<-matrix(nrow=boots, ncol=nrow(input))
maxmin.sim<-matrix(nrow=boots, ncol=nrow(input))
...
for(r in 1:boots){
  ...
  inputboot<-input[,c("id","from","to","distancekm") ]
  
  #set up functions to predict
  ...
  #collect pattern features for each o-d pair
  ...
  #put estimate of maximum time and value in a matrix
...
#get estimate of 95% CI for each group
...
#95% CI for average speed function
...
averagespeedreal<-function(time){
  nlinks<-nrow(input)
  output<-rep(0,length(time))
  for(i in 1:nrow(input)){
    distance<-input$distancekm[i]
    output<-output+distance/(a[[i]](time)/60)
  }
  output<-output/(nlinks)
  return(output)
}
...
input.real<-unique(journeytimes[,c("id","start.station_id","end.station_id","distancekm")])
input.real<-input.real[order(input.real$id),]
colnames(input.real)<-c("id","from","to","distancekm")
...
#plot of average speed (modelled and simulated)
linetype=as.factor(c("MLM","95%CI","SIM"))
colour=as.factor(c("MLM","95%CI","SIM"))
avspeed_mixed<-ggplot(data=avspeed.conf)+
  geom_line(aes(x=time, y=qlow, linetype=linetype[2], colour=colour[2]))+
  geom_line(aes(x=time, y=qhigh, linetype=linetype[2], colour=colour[2]))+
  stat_function(fun=averagespeed,aes( linetype=linetype[1], colour=colour[1]))+
  stat_function(fun=averagespeedreal, aes(  linetype=linetype[3], colour=colour[3]))+
  scale_colour_manual(breaks=c("MLM","95%CI","SIM"),values=c("grey30","black","red1"), name="Model")+
  scale_linetype_manual(breaks=c("MLM","95%CI","SIM"),values=c("solid", "dotted", "dashed"),name="Model")+
  scale_x_continuous(limits=c(0,24), breaks=seq(from=0,to=24,by=3), labels=paste(seq(from=0,to=24,by=3),"00",sep=":"))+
  xlab(label="Time")+
...
#plot of modelled average speed
avspeed_mlm<-ggplot(data=avspeed.conf)+
  ...
  stat_function(fun=averagespeed, lwd=1, colour="black")+
  theme_minimal()+
  xlab(label="Time")+
  ylab(label="Average journey speed (km/h)")+
  scale_x_continuous(limits=c(0,24), breaks=seq(from=0,to=24,by=3), labels=paste(seq(from=0,to=24,by=3),"00",sep=":"))+
...
#print speed function
png(paste("speedfunction_real",fileref, ".png", sep="_"), width=width, height=width*5/7, units="mm", res=600)
avspeed_mixed
dev.off()

#plot points and CIs for maximums
png(paste("maxpoints",fileref, ".png", sep="_"), width=width, height=width*5/7, units="mm", res=600)
print(ggplot(data=input, aes(colour=as.factor(id)))+
...
print(ggplot(data=basisplot, aes(x=time, y=basis, group=basisno, colour=as.factor(basisno), linetype=as.factor(basisno)), lwd=1)+
        geom_line()+
        theme_minimal()+
        theme(legend.position="none")+
        scale_x_continuous(limits=c(0,24)-centre, breaks=seq(from=0,to=24,by=3)-centre, labels=paste(seq(from=0,to=24,by=3),"00",sep=":"))+
        xlab(label="Time")+
        ylab(label="Basis Function Value")+
        theme(axis.text.x=element_text(angle=45,hjust=1)))
dev.off()

#save pattern features estimates
colnames(input.real)<-paste(colnames(input.real), "real",sep="_")
input<-merge(input, input.real, by.x=c("from","to"), by.y=c("from_real","to_real"))
...
for(i in 1:nc){
  if(i==1){
    tracemat<-tracearray[,i,]
    tracemat<-cbind(tracemat, iter=c(1:nrow(tracemat)), chain=rep(i,nrow(tracemat)))
  } else{
    tracematadd<-tracearray[,i,]
    tracematadd<-cbind(tracematadd, iter=c(1:nrow(tracematadd)), chain=rep(i,nrow(tracematadd)))
    tracemat<-rbind(tracemat,tracematadd)
    
  }
}

tracemat<-as.data.frame(tracemat)
write.csv(tracemat,paste("tracedata",fileref,".csv",sep=""), row.names=F)
